  ...
  let post_options = {
   ...
   try {
     ...
     let response = switch response_store.response {
      ...
      let email = switch json.email {
        | Some(email) => email
        | None => { Console.log("Email field not found while destructuring") ; ""}
      }
     ...
     if status == 400 {
       dialog.innerText = Null.make("Requisição inválida. Os dados informados estão corretos?")
       () // TODO: should exit the function here, does it?
     } else if status == 409 {
       dialog.innerText = Null.make("Já existe uma conta com este email")
       () // TODO: should exit the function here, does it?
      } else if status == 200 || status == 201 {
        Console.log(JSON.stringifyAny(credentials))
        store(storage, "mirante_credentials", credentials_stringified)
        dialog.innerText = Null.make("Conta criada com sucesso")
        () // TODO: should exit the function here, does it?
     }
   } catch {
     | _ => {
